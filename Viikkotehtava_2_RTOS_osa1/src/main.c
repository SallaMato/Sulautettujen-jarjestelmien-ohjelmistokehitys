...
volatile int tila = 0; // 0 = idle, 1 = red, 2 = yellow, 3 = green 4 = pause 5= yellow blink 6 = red manual 7 = yellow manual 8 = green manual
volatile int suunta = 0; //0=alas 1=ylös
volatile int tallennettu_tila = 0; // tallennettu tila
...
void button_2_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
    ...
    if (tila == 4) { // Toimii vain pausella
        tila = 7;
    } else {
        tila = 4;
    }
...
void yellow_led_task(void *, void *, void*) {
	...
	while (true) {
		...
		if (tila == 5) {
            // Sytytetään keltainen
            gpio_pin_set_dt(&red,1);
            gpio_pin_set_dt(&green,1);
            
            // Nukutaan vain 500ms (puoli sekuntia), jotta se vilkkuu nopeammin
            k_sleep(K_MSEC(500));
            
            // Sammutetaan keltainen
            gpio_pin_set_dt(&red,0);
            gpio_pin_set_dt(&green,0);
