...
static void uart_task (void *unused1, void *unused2, void *unused3) {
    ...
    while (true) {
        ...
        if (uart_poll_in(uart_dev, &rc) == 0) {
            ...
            //Jos kirjain ei ole X, lisätään se sanaan
            if (rc != 'X') {
                uart_msg[uart_msg_cnt] = rc;
                uart_msg_cnt++;
            }
            //Jos vastaanotettiin X, sana on valmis lähetettäväksi
...
static void dispatcher_task(void *unused1, void *unused2, void *unused3) {
    ...
    while (true) {
		...
		//Katsotaan onko ensimmäinen merkki numero
		if (isdigit((unsigned char)sequence[0])) {
			int parsed_seconds = time_parse(sequence);
			//Lisäys robottia varten. Lähetetään vastaus takaisin uartiin
		...
		// Lisäys robottia varten. Lähetetään sekvenssin testitulos UARTiin
